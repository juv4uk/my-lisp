...
6. 🚧 **NLP-міст (`understand`), перша версія.** Частково зроблено, статус — експериментальний: `crates/my-lisp-cli/src/llm.rs` + команди REPL `:tell`/`:ask`, задокументовано в `docs/vision.md` і doc-коментарях. LLM лише перекладає природну мову в Lisp-синтаксис; `tell-knowledge`/`reason-in` роблять реальну символьну перевірку. Не покрито тестами (реальний зовнішній API) — свідомо, згідно `docs/testing.md`. Це перший крок трійці `eval`/`reason`/`understand` з `private/lisp-to-knowledge.md` §9.
...
8. ⬜ **Атом як вхід у поняття (concept entry point).** Не почато. Ідея з `private/lisp-to-knowledge.md` §4: символ на кшталт `земля` міг би розкриватися у структуровану картку властивостей (`(тип планета) (супутник місяць) ...`), а не лишатися голим ім'ям. На практиці це, ймовірно, лягає на вже наявну модель `defmodule`/фактів у `lib/knowledge.my`, а не вимагає нового примітиву — радше конвенція запису фактів і, можливо, одна довідкова функція (`describe`/`concept-of`). Обговорити з користувачем обсяг перед стартом.
