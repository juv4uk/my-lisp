...
3. ✅ **Невеликий символьний AI-приклад.** Зроблено: [`lib/unify.my`](../lib/unify.my) — unification (без occurs-check, свідомо, стандартне спрощення для навчального unifier'а), логічні змінні як `(var name)`-пари (не голі символи — `eq` вимагає атомів, немає `symbol?`), 9 тестів у `crates/my-lisp/tests/unify.rs`. Ручним тестуванням до фіксації дизайну зловлено 2 реальні баги — `var?` і `unify` обидва безумовно викликали `eq` там, де один з операндів міг виявитись списком, не атомом; виправлено guard `(atom ...)` перед кожним `eq`.
