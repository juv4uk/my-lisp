...
1. **Метациркулярний evaluator самою my-lisp.** Написати `(my-eval expr env)` в `lib/core.my` (чи окремому файлі), що інтерпретує quoted-списки як код через `read`/`car`/`cdr`/`cond`/`lambda`, за схемою eval/apply з оригінальної статті Маккарті 1960 року. Не для продукту — для демонстрації того, що мова справді може описати сама себе, буквально, не метафорично.
2. **`tests/fixtures/conformance.json` — тримати як єдину точку правди**, коли з'явиться C-ядро чи HDL-реалізація (див. CLAUDE.md, "Confirmed future direction"). Не давати нових реалізацій ставати "ще одним несумісним діалектом" — саме ця фрагментація Lisp-екосистеми 70-80х засмучувала Маккарті найбільше.
3. **Невеликий символьний AI-приклад** (unifier чи backward-chaining resolver, кілька десятків рядків my-lisp) — у дусі "Advice Taker" 1958 року, для якого Lisp взагалі задумувався. Не інфраструктурна робота, радше демонстраційна/навчальна.
4. **Продовжувати опір розширенню Rust-ядра**, коли наявних примітивів достатньо (як було з `<=`/`>=` — два рядки, не нова машинерія). Той самий інстинкт, що змусив Маккарті не добудовувати M-expressions, коли S-вирази виявились достатніми.
