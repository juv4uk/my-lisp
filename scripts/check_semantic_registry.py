...
SID серіалізується як string з рівно 8 біт. UK/EN/SA є рівноправними людськими поверхнями,
...
def check(root) -> tuple[int, set[str], int]:
    ...
    for entry in root[1:]:
        ...
        if sid != len(seen_ids):
            raise ValueError(
                f"SID axis мусить бути щільною й впорядкованою: очікував {len(seen_ids):08b}, отримав {bits}"
            )
        ...
        if sid == 0:
            if entry != [identity, []]:
                raise ValueError("SID 00000000 мусить бути рівно ground row (\"00000000\" ())")
            continue
