...
Behavior prose lives once in lib/surface/uk-docs.wsm, keyed by numeric semantic ID.
Surface spellings/statuses come from lib/generated/function-table.wsm, itself a projection
of lib/surface/semantic-registry.wsm. This script only joins those two projections.
"""
...
DOCS_INDEX = ROOT / "lib/surface/uk-docs.wsm"
FUNCTION_TABLE = ROOT / "lib/generated/function-table.wsm"
...
def parse_docs_index() -> list[DocRow]:
    ...
    if "(schema uk-api-docs/3)" not in source:
        raise SystemExit("uk-docs.wsm must use schema uk-api-docs/3 before generation")
...
def render_reference(rows: list[DocRow], table: dict[str, dict[str, Surface]]) -> str:
    ...
    out = [
        ...
        "",
        "Нижче — згенерований join по **numeric semantic ID**. Опис поведінки береться один раз із `lib/surface/uk-docs.wsm`; `uk`, `ukr`, статус `ukr` та основа беруться з authoritative function-table projection. Ручне редагування рядків цієї секції буде перезаписано генератором.",
        "",
    ]
