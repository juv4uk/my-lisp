...
- Not every true-today behavior belongs here, though: if it's a genuinely open scope question whether a *future* implementation should still behave that way — `Rational`'s `i64` (not bignum) overflow ceiling is the concrete example, see `docs/language-core.md` — putting it in this append-only, never-edited file would prematurely settle that question by fixture rather than by decision. Implementation-specific ceilings like that belong in that implementation's own test suite (e.g. `crates/my-lisp/tests/mccarthy.rs`'s `exact_arithmetic_overflow_fails_loudly_not_silently`), not here.
...
- Але не кожна істинна-сьогодні поведінка тут доречна: якщо це справді відкрите питання обсягу, чи *майбутня* реалізація має поводитись так само — стеля переповнення `i64` (не bignum) у `Rational` є конкретним прикладом, див. `docs/language-core.md` — покласти це в цей append-only, ніколи не редагований файл означало б передчасно вирішити це питання фікстурою, а не рішенням. Реалізаційно-специфічні межі на кшталт цієї належать до тестового набору самої реалізації (напр. `exact_arithmetic_overflow_fails_loudly_not_silently` у `crates/my-lisp/tests/mccarthy.rs`), не сюди.
...
- Nicht jedes heute-wahre Verhalten gehört jedoch hierher: wenn es eine echte offene Scope-Frage ist, ob eine *künftige* Implementierung sich weiterhin so verhalten sollte — `Rational`s `i64`- (nicht Bignum-) Überlaufgrenze ist das konkrete Beispiel, siehe `docs/language-core.md` — würde es diese Frage per Fixture statt per Entscheidung vorzeitig festlegen, sie in diese append-only, nie bearbeitete Datei zu legen. Implementierungsspezifische Grenzen wie diese gehören in die eigene Testsuite dieser Implementierung (z. B. `exact_arithmetic_overflow_fails_loudly_not_silently` in `crates/my-lisp/tests/mccarthy.rs`), nicht hierher.
