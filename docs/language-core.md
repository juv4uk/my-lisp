...
`<`, `>`, `=`, `<=`, `>=` cover ordering and equality comparison. `print`, `read`, and `eval` close the classic McCarthy read/eval/print loop by hand, from inside the language itself — `read` deliberately takes a string, not stdin, so the crate stays capability-free. Interactive input remains the one open item: it's a separate host-boundary primitive with its own explicit design decision to make, not an assumption bolted onto `read`.

`<`, `>`, `=`, `<=`, `>=` покривають впорядкування й рівність. `print`, `read` і `eval` вручну замикають класичний McCarthy read/eval/print цикл зсередини самої мови — `read` навмисно приймає рядок, не stdin, тож крейт лишається без host-можливостей. Інтерактивний ввід лишається єдиним відкритим пунктом: це окремий host-межовий примітив зі своїм явним архітектурним рішенням, яке ще треба ухвалити, не припущення, приліплене до `read`.

`<`, `>`, `=`, `<=`, `>=` decken Ordnung und Gleichheit ab. `print`, `read` und `eval` schließen die klassische McCarthy-Read/Eval/Print-Schleife von Hand, aus der Sprache selbst heraus — `read` nimmt bewusst eine Zeichenkette entgegen, nicht stdin, damit das Crate ohne Host-Fähigkeiten bleibt. Interaktive Eingabe bleibt der eine offene Punkt: ein separates Host-Grenz-Primitiv mit einer noch zu treffenden expliziten Designentscheidung, keine an `read` angeflanschte Annahme.
