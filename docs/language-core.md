...
`<`, `>`, `=` cover strict-order and equality comparison; there is no `<=`/`>=` yet (trivially addable the same way if needed) and no I/O primitives (no `print`/`display`/`read`) at any layer — the CLI only ever shows the *last* top-level form's value, everything before it runs purely for `def`/`defmacro` side effects. `Session`/`EvalResult` already carry an `output` field reserved for a future print-like builtin, but nothing populates it yet. `read` in particular is an open architectural question, not a missing detail: whether it means parsing a datum from a string (capability-free, fits the existing parser) or reading interactive input (a real host boundary, breaking the crate's "no direct file/network/UI access" contract) needs a decision before implementation, not an assumption.

`<`, `>`, `=` покривають строге впорядкування й рівність; `<=`/`>=` поки немає (тривіально додати так само, коли знадобиться), і немає I/O-примітивів (немає `print`/`display`/`read`) на жодному рівні — CLI показує лише значення *останньої* верхньорівневої форми, все перед нею виконується лише заради побічних ефектів `def`/`defmacro`. `Session`/`EvalResult` вже мають поле `output`, зарезервоване під майбутній print-подібний built-in, але поки нічого його не заповнює. `read` зокрема — відкрите архітектурне питання, не деталь, якої бракує: чи означає він парсинг даних з рядка (без host-можливостей, вписується в наявний parser), чи інтерактивне читання вводу (реальна host-межа, що ламає контракт крейта "без прямого доступу до файлів/мережі/UI") — рішення потрібне до реалізації, не припущення.

`<`, `>`, `=` decken strikte Ordnung und Gleichheit ab; `<=`/`>=` gibt es noch nicht (bei Bedarf trivial auf dieselbe Weise ergänzbar), und es gibt keine I/O-Primitive (kein `print`/`display`/`read`) auf irgendeiner Ebene — die CLI zeigt nur den Wert der *letzten* Top-Level-Form, alles davor läuft rein wegen `def`/`defmacro`-Nebeneffekten. `Session`/`EvalResult` tragen bereits ein `output`-Feld für einen künftigen print-artigen Built-in, aber nichts füllt es bisher. `read` insbesondere ist eine offene architektonische Frage, kein fehlendes Detail: ob es das Parsen eines Datums aus einer Zeichenkette bedeutet (ohne Host-Fähigkeiten, passt zum vorhandenen Parser) oder interaktives Einlesen (eine echte Host-Grenze, die den "kein direkter Datei-/Netzwerk-/UI-Zugriff"-Vertrag des Crates bricht), muss vor der Implementierung entschieden werden, nicht angenommen.
