...
`<`, `>`, `=` cover strict-order and equality comparison; there is no `<=`/`>=` yet (trivially addable the same way if needed). `print` covers output; there is still no `read` — the CLI's own top-level-form printing (only the *last* form's value, past `print`'s accumulated transcript) is unrelated to reading input back in. `read` is an open architectural question, not a missing detail: whether it means parsing a datum from a string (capability-free, fits the existing parser) or reading interactive input (a real host boundary, breaking the crate's "no direct file/network/UI access" contract) needs a decision before implementation, not an assumption.

`<`, `>`, `=` покривають строге впорядкування й рівність; `<=`/`>=` поки немає (тривіально додати так само, коли знадобиться). `print` покриває вивід; досі немає `read` — власний друк CLI верхньорівневих форм (лише значення *останньої* форми, поза накопиченим транскриптом `print`) не пов'язаний із читанням вводу назад. `read` — відкрите архітектурне питання, не деталь, якої бракує: чи означає він парсинг даних з рядка (без host-можливостей, вписується в наявний parser), чи інтерактивне читання вводу (реальна host-межа, що ламає контракт крейта "без прямого доступу до файлів/мережі/UI") — рішення потрібне до реалізації, не припущення.

`<`, `>`, `=` decken strikte Ordnung und Gleichheit ab; `<=`/`>=` gibt es noch nicht (bei Bedarf trivial auf dieselbe Weise ergänzbar). `print` deckt Ausgabe ab; noch immer gibt es kein `read` — die eigene Ausgabe der CLI für Top-Level-Formen (nur der Wert der *letzten* Form, über das angesammelte Transkript von `print` hinaus) hat nichts mit dem Zurücklesen von Eingaben zu tun. `read` ist eine offene architektonische Frage, kein fehlendes Detail: ob es das Parsen eines Datums aus einer Zeichenkette bedeutet (ohne Host-Fähigkeiten, passt zum vorhandenen Parser) oder interaktives Einlesen (eine echte Host-Grenze, die den "kein direkter Datei-/Netzwerk-/UI-Zugriff"-Vertrag des Crates bricht), muss vor der Implementierung entschieden werden, nicht angenommen.
