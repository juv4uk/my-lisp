...
- `read` — McCarthy's original reader primitive: evaluates its one string argument and parses it into exactly one s-expression of *data*, the same way `'expr` does, without evaluating it. Errors (`ErrorKind::Type`) if the argument isn't a string, or (`ErrorKind::InvalidForm`) if the string holds zero or more than one top-level expression. · Оригінальний reader-примітив Маккарті: обчислює свій єдиний рядковий аргумент і парсить його в рівно одну s-expression *даних*, так само як `'expr`, без обчислення. Помилка (`ErrorKind::Type`), якщо аргумент не рядок, або (`ErrorKind::InvalidForm`), якщо рядок містить нуль чи більше одного верхньорівневого виразу. · McCarthys ursprüngliches Reader-Primitiv: wertet sein einziges String-Argument aus und parst es in genau eine s-Expression aus *Daten*, genau wie `'expr`, ohne sie auszuwerten. Fehler (`ErrorKind::Type`), wenn das Argument keine Zeichenkette ist, oder (`ErrorKind::InvalidForm`), wenn die Zeichenkette null oder mehr als einen Top-Level-Ausdruck enthält.
...
`<`, `>`, `=` cover strict-order and equality comparison; there is no `<=`/`>=` yet (trivially addable the same way if needed). `print`, `read`, and `eval` close the classic McCarthy read/eval/print loop by hand, from inside the language itself — `read` deliberately takes a string, not stdin, so the crate stays capability-free; interactive input, if it's ever added, is a separate host-boundary primitive with its own explicit design decision, not an assumption bolted onto `read`.

`<`, `>`, `=` покривають строге впорядкування й рівність; `<=`/`>=` поки немає (тривіально додати так само, коли знадобиться). `print`, `read` і `eval` вручну замикають класичний McCarthy read/eval/print цикл зсередини самої мови — `read` навмисно приймає рядок, не stdin, тож крейт лишається без host-можливостей; інтерактивний ввід, якщо колись з'явиться, — окремий host-межовий примітив зі своїм явним архітектурним рішенням, не припущення, приліплене до `read`.

`<`, `>`, `=` decken strikte Ordnung und Gleichheit ab; `<=`/`>=` gibt es noch nicht (bei Bedarf trivial auf dieselbe Weise ergänzbar). `print`, `read` und `eval` schließen die klassische McCarthy-Read/Eval/Print-Schleife von Hand, aus der Sprache selbst heraus — `read` nimmt bewusst eine Zeichenkette entgegen, nicht stdin, damit das Crate ohne Host-Fähigkeiten bleibt; interaktive Eingabe, falls je hinzugefügt, ist ein separates Host-Grenz-Primitiv mit eigener expliziter Designentscheidung, keine an `read` angeflanschte Annahme.
