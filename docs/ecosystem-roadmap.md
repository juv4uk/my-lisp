...
2. ✅ `cml` has `compatibility.lisp` with my-lisp contract `1.0`, target ISA, tested SHAs, features, and limitations.
...
9. Compiled Lisp grows by value: a small `core.lisp` subset, then `unify.lisp`, then `reason.lisp`.
...
Правило порядку — за цінністю: спочатку коректні й переносні source semantics та Advice Taker observations у `my-lisp`; далі CML/FPGA розширюються лише там, де незалежне виконання підсилює conformance або реально веде `core.lisp → unify.lisp → reason.lisp` до другого backend. Нова ISA surface сама по собі не є метою.
...
2. ✅ `cml` має `compatibility.lisp` із контрактом my-lisp `1.0`, цільовою ISA, перевіреними SHA, features і обмеженнями.
...
9. Compiled Lisp росте за цінністю: малий subset `core.lisp`, потім `unify.lisp`, потім `reason.lisp`.
...
2. ✅ `cml` besitzt `compatibility.lisp` mit my-lisp-Vertrag `1.0`, Ziel-ISA, geprüften SHAs, Features und Grenzen.
...
9. Kompiliertes Lisp wächst nach Nutzen: ein kleiner `core.lisp`-Teil, dann `unify.lisp`, dann `reason.lisp`.
