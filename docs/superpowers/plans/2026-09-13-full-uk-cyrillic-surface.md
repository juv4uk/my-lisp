...
**Goal:** Make `ukr` an authoritative, executable, Cyrillic-only peer surface that can be typed without switching away from the Ukrainian keyboard layout.

**Architecture:** Keep numeric semantic IDs as the only identity and `semantic-registry.wsm` as the only spelling authority. Add `ukr` triples to registry rows, have all existing generic registry consumers resolve them automatically, make the function-table generator read them instead of synthesizing them, and enforce the no-Latin rule with executable tests. Preserve current `uk` spellings unchanged.

**Tech Stack:** my-lisp/WASM-style source contracts, Rust integration tests/xtask checks, existing CLI/evaluator, GitHub Actions.

**Spec:** `docs/superpowers/specs/2026-09-13-ukr-cyrillic-surface-design.md`
...
- Admitted `ukr` identifiers may contain Ukrainian Cyrillic letters, digits, and admitted Lisp identifier punctuation, but no ASCII Latin letters.
...
- Generated tables are projections and must not invent `ukr` names.
...
- Produces: a regression test that rejects every staging `ukr` candidate containing ASCII Latin letters.
...
Update only the offending ukr candidate field(s). Remove the `дозволені-міжнародні-позначення` policy that permits Latin identifier fragments for `ukr`; replace it with the explicit no-layout-switch rule. Do not change current stable `uk` spellings.
...
### Task 2: Admit `ukr` as a real registry namespace
...
- Produces: admitted `(ukr NAME stable)` triples that resolve to the same numeric semantic IDs as peer spellings.

- [ ] **Step 1: Write the failing registry test**

Add a synthetic parser test proving namespace `ukr` is preserved and a live-registry test proving one ratified ukr spelling resolves to the same semantic ID as its current `uk` peer.
...
Expected: the live ukr assertion FAILS because the registry has no authoritative `ukr` triples yet.

- [ ] **Step 3: Add authoritative `ukr` triples without changing `uk`**

For the first ratified slice, add `(ukr ... stable)` to semantically clear identities. Existing `uk stable` names that are already full-word Ukrainian may use the same spelling. For abbreviated/current names, use the reviewed staging full-word spelling only when it passes the Cyrillic-only rule.
...
### Task 3: Stop generated tables from inventing `ukr`
...
**Interfaces:**
- Consumes: registry `(ukr WORD STATUS)` triples.
- Produces: generated `ukr` columns exactly matching registry authority.

- [ ] **Step 1: Write the failing projection test**

Add a test that selects a registry row where `uk != ukr` and asserts the generated function table contains the authoritative `ukr` spelling/status rather than a mirror of `uk`.

- [ ] **Step 2: Run focused test and verify RED**

Run the UK audit integration test; expected FAIL because `ukr-projection` currently mirrors `uk`.

- [ ] **Step 3: Change generator to read `ukr` directly**

In `generate-function-table.my`, call `get-surface` for `ukr`; remove the mirroring `ukr-projection` policy. Preserve `— missing` when no ukr row exists. Never infer a name.
...
### Task 4: Prove the meta/native runtime sees the same ukr identity
...
- Produces: native and meta-evaluator identity parity for a `ukr` spelling.

- [ ] **Step 1: Add failing peer-identity test**

Choose a ratified ukr spelling that differs from current `uk`, resolve/evaluate both spellings, and assert both materialize the same semantic callable identity.
...
Run the existing generator/check path so admitted `ukr` triples are projected generically. Do not add a separate ukr dictionary.
...
**Files:**
- Create: `lib/surface/ukr-acceptance.lisp`
- Modify: `crates/my-lisp/tests/uk_sa_surface.rs`
- Modify: `crates/xtask/src/checks.rs`

**Interfaces:**
- Consumes: admitted `ukr` registry names.
...
Add a runner test that evaluates `ukr-acceptance.lisp` and expects symbol `успіх`. Add an xtask policy check that strips comments/string contents and rejects ASCII Latin letters in executable code.
...
Exercise definition, function, condition, arithmetic, list processing, a higher-order operation, strings, and one non-core/host capability through ratified `ukr` names. Keep all executable alphabetic identifiers Ukrainian Cyrillic.
...
```bash
git add lib/surface/ukr-acceptance.lisp crates/my-lisp/tests/uk_sa_surface.rs crates/xtask/src/checks.rs
...
Review the diff: no semantic IDs changed; no existing stable `uk` spelling disappeared; generated files match authority; `ukr` admitted identifiers contain no ASCII Latin letters.
