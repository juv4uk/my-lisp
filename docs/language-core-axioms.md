...
- **`NIL`/`false` conflation** — my-lisp currently follows Lisp 1.5's original choice (`'()` and false are the same value; see `(cond (() 'wrong) (t 'right))` in `conformance.json`). This is inherited deliberately, not by accident — Scheme's later split of `'()`/`#f` is a different, equally legitimate design, not a correction of an error. Whether to keep the conflation is a real design decision this project has already made, just not yet stated as an axiom.
  **Суміщення `NIL`/`false`** — my-lisp зараз дотримується оригінального вибору Lisp 1.5 (`'()` і хиба — те саме значення; див. `(cond (() 'wrong) (t 'right))` у `conformance.json`). Це успадковано свідомо, не випадково — пізніший розділ `'()`/`#f` у Scheme — інший, так само законний дизайн, не виправлення помилки. Чи зберігати суміщення — реальне дизайн-рішення, яке проєкт уже ухвалив, лише ще не сформульоване як аксіома.
