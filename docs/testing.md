...
| **Total** | | **135** | | **135 passed, 0 failed, 0 ignored** |
...
| `my-lisp` | `tests/forward.rs` | 8 | `lib/forward.my`, Кроки 1-3 forward-chaining рушія в стилі CLIPS: `assert-fact!` — розширення глобальної working memory, `fire-rule` — унікація шаблону правила з одним фактом, `fire-rule-on-facts`/`fire-rule-on-working-memory` — застосування одного правила до всього списку фактів, `fire-rules-on-facts`/`fire-rules-on-working-memory` — застосування всього набору правил за один прохід | ok |
...
| **Разом** | | **135** | | **135 пройдено, 0 провалів, 0 пропущено** |
...
| `my-lisp` | `tests/forward.rs` | 8 | `lib/forward.my`, Schritte 1-3 einer CLIPS-artigen Forward-Chaining-Engine: `assert-fact!` erweitert die globale Working Memory, `fire-rule` unifiziert das Muster einer Regel mit einem Fakt, `fire-rule-on-facts`/`fire-rule-on-working-memory` wenden eine Regel auf eine ganze Faktenliste an, `fire-rules-on-facts`/`fire-rules-on-working-memory` wenden ein ganzes Regelwerk in einem Durchlauf an | ok |
...
| **Gesamt** | | **135** | | **135 bestanden, 0 fehlgeschlagen, 0 übersprungen** |
