...
| `my-lisp` | `tests/mccarthy.rs` | 25 | the seven McCarthy primitives, exact/inexact arithmetic, `<`/`>`/`=` comparison chaining, `print`'s session-wide output transcript (including through closures), `read`/`eval` closing the read-eval loop by hand, lambda semantics, structured errors, `lib/core.my` list utilities (`length`, `reverse`, `append`, `map`, `filter`, `reduce`), `let`/`let*`, `equal?` | ok |
...
| **Total** | | **90** | | **90 passed, 0 failed, 0 ignored** |

The implementation-independent conformance fixture at [`tests/fixtures/conformance.json`](../tests/fixtures/conformance.json) — its own format and rules are in [`tests/fixtures/README.md`](../tests/fixtures/README.md) — is included directly into `crates/my-lisp/tests/mccarthy.rs` via `include_str!` and is exercised as part of that suite's 25 tests, not counted separately.
...
| **Разом** | | **90** | | **90 пройдено, 0 провалів, 0 пропущено** |

Незалежна від реалізації conformance-фікстура [`tests/fixtures/conformance.json`](../tests/fixtures/conformance.json) — власний формат і правила описані в [`tests/fixtures/README.md`](../tests/fixtures/README.md) — підключається напряму в `crates/my-lisp/tests/mccarthy.rs` через `include_str!` і перевіряється в межах тих 25 тестів набору, окремо не рахується.
