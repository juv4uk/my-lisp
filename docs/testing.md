...
| `my-lisp` | `tests/mccarthy.rs` | 18 | the seven McCarthy primitives, exact/inexact arithmetic, `<`/`>`/`=` comparison chaining, `print`'s session-wide output transcript (including through closures), lambda semantics, structured errors, `lib/core.my` list utilities (`length`, `reverse`, `append`, `map`, `filter`, `reduce`) | ok |
...
| **Total** | | **63** | | **63 passed, 0 failed, 0 ignored** |
...
| `my-lisp` | `tests/mccarthy.rs` | 18 | сім примітивів Маккарті, точну/неточну арифметику, ланцюгове порівняння `<`/`>`/`=`, транскрипт виводу `print`, спільний на сесію (включно через замикання), семантику lambda, структуровані помилки, list-утиліти `lib/core.my` (`length`, `reverse`, `append`, `map`, `filter`, `reduce`) | ok |
...
| **Разом** | | **63** | | **63 пройдено, 0 провалів, 0 пропущено** |
...
| `my-lisp` | `tests/mccarthy.rs` | 18 | die sieben McCarthy-Primitive, exakte/inexakte Arithmetik, verkettete Vergleiche `<`/`>`/`=`, `print`s sitzungsweites Ausgabetranskript (auch durch Closures hindurch), Lambda-Semantik, strukturierte Fehler, `lib/core.my`-Listenwerkzeuge (`length`, `reverse`, `append`, `map`, `filter`, `reduce`) | ok |
...
| **Gesamt** | | **63** | | **63 bestanden, 0 fehlgeschlagen, 0 übersprungen** |
