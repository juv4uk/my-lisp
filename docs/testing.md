...
| **Total** | | **133** | | **133 passed, 0 failed, 0 ignored** |
...
| `my-lisp` | `tests/forward.rs` | 6 | `lib/forward.my`, Кроки 1-2 forward-chaining рушія в стилі CLIPS: `assert-fact!` — розширення глобальної working memory, `fire-rule` — унікація шаблону правила з одним фактом, `fire-rule-on-facts`/`fire-rule-on-working-memory` — застосування одного правила до всього списку фактів зі збором нових фактів | ok |
...
| **Разом** | | **133** | | **133 пройдено, 0 провалів, 0 пропущено** |
...
| `my-lisp` | `tests/forward.rs` | 6 | `lib/forward.my`, Schritte 1-2 einer CLIPS-artigen Forward-Chaining-Engine: `assert-fact!` erweitert die globale Working Memory, `fire-rule` unifiziert das Muster einer Regel mit einem Fakt, `fire-rule-on-facts`/`fire-rule-on-working-memory` wenden eine Regel auf eine ganze Faktenliste an und sammeln die erzeugten neuen Fakten | ok |
...
| **Gesamt** | | **133** | | **133 bestanden, 0 fehlgeschlagen, 0 übersprungen** |
