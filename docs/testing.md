...
| `my-lisp` | `tests/forward.rs` | 11 | `lib/forward.my`, Steps 1-4 of a CLIPS-style forward-chaining engine: `assert-fact!` growing the global working memory, `fire-rule` unifying a rule's pattern against one fact, `fire-rule-on-facts`/`fire-rule-on-working-memory` applying one rule across a whole fact list, `fire-rules-on-facts`/`fire-rules-on-working-memory` applying an entire rule set in one pass, `run` looping rules to a fixpoint (chained derivation across passes, no infinite loop on a self-reproducing rule), `assert-facts!` merging `run`'s result back into `*working-memory*` | ok |
...
| **Total** | | **138** | | **138 passed, 0 failed, 0 ignored** |
...
| `my-lisp` | `tests/forward.rs` | 11 | `lib/forward.my`, Кроки 1-4 forward-chaining рушія в стилі CLIPS: `assert-fact!` — розширення глобальної working memory, `fire-rule` — унікація шаблону правила з одним фактом, `fire-rule-on-facts`/`fire-rule-on-working-memory` — застосування одного правила до всього списку фактів, `fire-rules-on-facts`/`fire-rules-on-working-memory` — застосування всього набору правил за один прохід, `run` — цикл до fixpoint (ланцюгове виведення через кілька проходів, без нескінченного циклу на правилі, що відтворює вже наявний факт), `assert-facts!` — злиття результату `run` назад у `*working-memory*` | ok |
...
| **Разом** | | **138** | | **138 пройдено, 0 провалів, 0 пропущено** |
...
| `my-lisp` | `tests/forward.rs` | 11 | `lib/forward.my`, Schritte 1-4 einer CLIPS-artigen Forward-Chaining-Engine: `assert-fact!` erweitert die globale Working Memory, `fire-rule` unifiziert das Muster einer Regel mit einem Fakt, `fire-rule-on-facts`/`fire-rule-on-working-memory` wenden eine Regel auf eine ganze Faktenliste an, `fire-rules-on-facts`/`fire-rules-on-working-memory` wenden ein ganzes Regelwerk in einem Durchlauf an, `run` läuft bis zum Fixpunkt (verkettete Ableitung über mehrere Durchläufe, keine Endlosschleife bei einer sich selbst reproduzierenden Regel), `assert-facts!` führt das `run`-Ergebnis in `*working-memory*` zusammen | ok |
...
| **Gesamt** | | **138** | | **138 bestanden, 0 fehlgeschlagen, 0 übersprungen** |
