...
This layer deliberately contains no Rust primitive. Its event representation is byte-for-byte compatible with the existing `(tell module clause)` / `(retract module clause)` journal. Once `world.my` is loaded, legacy `defmodule` is redefined as a thin wrapper over `world-tell-all`: it extracts the returned journal so existing readers retain their exact behavior. This is the first compatibility migration; the remaining global writers still await the same treatment.
...
Шар навмисно не додає Rust-примітивів. Формат подій точно сумісний із чинним журналом `(tell модуль clause)` / `(retract модуль clause)`. Після завантаження `world.my` старий `defmodule` перевизначається як тонка обгортка над `world-tell-all`: він дістає журнал із поверненого світу, тому всі чинні читачі зберігають точну поведінку. Це перша compatibility-міграція; решта глобальних writer-ів ще чекає такого самого переносу.
...
Diese Schicht fügt bewusst kein Rust-Primitiv hinzu. Ihr Ereignisformat ist mit dem bestehenden Journal `(tell modul clause)` / `(retract modul clause)` vollständig kompatibel. Nach dem Laden von `world.my` wird das alte `defmodule` als dünne Hülle um `world-tell-all` neu definiert: Es entnimmt der gelieferten Welt das Journal, sodass bestehende Leser ihr exaktes Verhalten behalten. Dies ist die erste Kompatibilitätsmigration; die übrigen globalen Writer folgen später.
