...
На базі аудиту malformed `node.lisp` перетворювався на empty S-expression і міг бути переписаний, а malformed/invalid records у `events.log` мовчки пропускалися. PR #34 переводить обидва authoritative recovery paths у fail-closed режим і додає контрприклади зі збереженням bytes. Валідний pre-M1.1a identity без `incarnation` залишається явним compatibility migration, а не помилково класифікується як corruption.
