...
**Згенеровано:** 2026-08-24 · база `e594fd0` · Vyasa
**Джерело:** live `(env)` через `scripts/gen-functions.my` (contract 2.1: builtins живуть у середовищі) + статичний прохід `lib/*.my`.
**Всього:** 23 builtin'ів + 421 бібліотечних функцій у 16 файлах.
...
`*`, `+`, `-`, `/`, `<`, `=`, `>`, `abs`, `atom`, `car`, `cdr`, `cons`, `env`, `eq`, `make-vector`, `max`, `max-list`, `min`, `min-list`, `vector`, `vector-length`, `vector-ref`, `vector-set!`
