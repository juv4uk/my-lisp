...
## ДОДАТОК 2026-08-23: верифікація гіпотези фантомного cdr (Р5 блокер)

Запит Vyasa: чи може "cdr expects non-empty list" бути замаскованим
stack overflow на глибокій cdr-ланцюгу?

**Мій тест:** tail-call build списку зі 100 000 елементів → car/cdr/
length-onto — чисто, нуль крахів, довжина точна.

**Факти про evaluator:**
- evaluate() = trampoline loop для TailCall (eval/mod.rs:67-85) —
  стек не росте на хвостовій рекурсії;
- Value має iterative Drop — дроп глибоких списків stack-safe;
- глибина 100K без обмежень і без фантомних помилок.

**Висновок:** гіпотеза stack-overflow НЕ підтверджена у базовому випадку.
Нова робоча гіпотеза (передано Vyasa): у ланцюгу normalize→subsample→
quantize→chamfer2 на раціональних з великими знаменниками виникає
ПОРОЖНІЙ список (edge case), і наступний cdr падає законно. Це баг
логіки даних WSM-24, не памʼяті. GC M0 знімається з блокерів за цим
пунктом — залишається телеметрія циклів.
