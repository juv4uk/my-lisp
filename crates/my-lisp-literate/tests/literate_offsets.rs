...
"#;
    ...
    let result = my_lisp_literate::eval_literate(source, &mut session).expect("should evaluate successfully");
...
fn test_fallback_no_markdown() {
    let source = "(+ 10 20)";
    let mut session = my_lisp::Session::default();
    let result = my_lisp_literate::eval_literate(source, &mut session).expect("should evaluate fallback");
...
"#;
    ...
    let mut session = my_lisp::Session::default();
    let error = my_lisp_literate::eval_literate(source, &mut session).expect_err("should fail on unknown symbol");
