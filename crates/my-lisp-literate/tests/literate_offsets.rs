...
"#;
    ...
    let result = my_lisp_literate::eval_literate(source, SourceMode::Literate, &mut session)
...
fn test_fallback_no_markdown() {
    let source = "(+ 10 20)";
    let mut session = my_lisp::Session::default();
    let result = my_lisp_literate::eval_literate(source, SourceMode::PureLisp, &mut session)
...
"#;
    ...
    let mut session = my_lisp::Session::default();
    let error = my_lisp_literate::eval_literate(source, SourceMode::Literate, &mut session)
...
fn test_literate_evaluation() {
    ...
    let mut session = my_lisp::Session::default();
    let (res, _) =
        my_lisp_literate::eval_literate(source, SourceMode::Literate, &mut session).unwrap();
