...
pub fn all() -> Vec<Check> {
    vec![
        ...
        Check {
            name: "smyslovyi-audyt-pokryvaie-vsi-140-stable-nazv",
            run: smyslovyi_audyt_pokryvaie_vsi_140_stable_nazv,
        },
        Check {
...
fn vsi_stable_ukrainski_nazvy_maiut_numeric_zapys_u_dovidnyku() -> Result<(), String> {
    ...
    if coverage.len() != 140 {
        return Err(format!("stable UK-покриття змінилося: {}", coverage.len()));
    }
    if docs.len() != 140 {
        return Err(format!("довідник мусить мати рівно 140 записів: {}", docs.len()));
    }
...
fn znak_pytannia_tochno_vidpovidaie_predykatam() -> Result<(), String> {
    ...
    if predicates != 30 {
        return Err(format!("змінився каталог публічних предикатів: {predicates}"));
    }
    Ok(())
}
...
fn znak_oklyku_tochno_vidpovidaie_mutatsii() -> Result<(), String> {
    ...
    if mutations != 1 {
        return Err(format!("публічний каталог мутацій змінився: {mutations}"));
    }
    Ok(())
}

fn smyslovyi_audyt_pokryvaie_vsi_140_stable_nazv() -> Result<(), String> {
    let mut problems = Vec::new();
    if !NAME_AUDIT.contains("(stable-reviewed 140)") {
        problems.push("missing (stable-reviewed 140)".to_string());
    }
    if !NAME_AUDIT.contains("(renamed 42)") {
        problems.push("missing (renamed 42)".to_string());
    }
    if !NAME_AUDIT.contains("(retained 98)") {
        problems.push("missing (retained 98)".to_string());
    }
    let rename_count = NAME_AUDIT
        .lines()
        .filter(|line| line.trim_start().starts_with("(rename "))
        .count();
    if rename_count != 42 {
        problems.push(format!("expected 42 (rename ...) lines, found {rename_count}"));
    }
    if problems.is_empty() {
        Ok(())
    } else {
