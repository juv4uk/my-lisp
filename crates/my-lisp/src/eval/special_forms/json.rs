...
pub(crate) fn evaluate_json_parse(
    ...
    parser.skip_ws();
    let value = parser.parse_value(span)?;
    parser.skip_ws();
    if parser.pos != parser.bytes.len() {
        return Err(parser.error("trailing characters after JSON value", span));
    }
    Ok(value)
}
...
impl<'a> JsonParser<'a> {
    fn error(&self, message: &str, span: Span) -> LanguageError {
        LanguageError::new(
    ...
    fn expect(&mut self, byte: u8, span: Span) -> Result<(), LanguageError> {
        if self.bump() == Some(byte) {
            Ok(())
        } else {
            Err(self.error(&format!("expected '{}'", byte as char), span))
        }
    }

    fn parse_value(&mut self, span: Span) -> Result<Value, LanguageError> {
        match self.peek() {
            Some(b'{') => self.parse_object(span),
            Some(b'[') => self.parse_array(span),
            Some(b'"') => self.parse_string(span).map(|s| Value::String(Rc::from(s.as_str()))),
            Some(b't') => self.parse_literal("true", Value::Bool(true), span),
            Some(b'f') => self.parse_literal("false", Value::Bool(false), span),
            Some(b'n') => self.parse_literal("null", Value::Nil, span),
            Some(c) if c == b'-' || c.is_ascii_digit() => self.parse_number(span),
            _ => Err(self.error("unexpected character", span)),
        }
    }

    fn parse_literal(&mut self, word: &str, value: Value, span: Span) -> Result<Value, LanguageError> {
        if self.bytes[self.pos..].starts_with(word.as_bytes()) {
            self.pos += word.len();
            Ok(value)
        } else {
            Err(self.error(&format!("invalid literal, expected {word}"), span))
        }
    }

    fn parse_object(&mut self, span: Span) -> Result<Value, LanguageError> {
        self.expect(b'{', span)?;
        ...
        loop {
            ...
            self.expect(b':', span)?;
            self.skip_ws();
            let value = self.parse_value(span)?;
            pairs.push((Value::String(Rc::from(key.as_str())), value));
            self.skip_ws();
            match self.bump() {
                Some(b',') => continue,
                Some(b'}') => break,
                _ => return Err(self.error("expected ',' or '}' in object", span)),
            }
        }
    ...
    fn parse_array(&mut self, span: Span) -> Result<Value, LanguageError> {
        self.expect(b'[', span)?;
        ...
        loop {
            self.skip_ws();
            items.push(self.parse_value(span)?);
            self.skip_ws();
            match self.bump() {
                Some(b',') => continue,
                Some(b']') => break,
                _ => return Err(self.error("expected ',' or ']' in array", span)),
            }
        }
        Ok(Value::list(items))
    }

    fn parse_string(&mut self, span: Span) -> Result<String, LanguageError> {
        self.expect(b'"', span)?;
        ...
        loop {
            match self.bump() {
                ...
                Some(b'\\') => match self.bump() {
                    ...
                    _ => return Err(self.error("invalid escape sequence", span)),
                },
                Some(_byte) => {
                    ...
                    match std::str::from_utf8(&self.bytes[start..end]) {
                        ...
                        Err(_) => return Err(self.error("invalid UTF-8 in string", span)),
                    }
                }
            }
        }
    ...
    fn parse_hex4(&mut self, span: Span) -> Result<u32, LanguageError> {
        let mut code: u32 = 0;
        for _ in 0..4 {
            let digit = match self.bump() {
                Some(b) if b.is_ascii_hexdigit() => (b as char).to_digit(16).unwrap(),
                _ => return Err(self.error("invalid \\u escape: expected 4 hex digits", span)),
            };
            code = code * 16 + digit;
        }
        Ok(code)
    ...
    fn parse_number(&mut self, span: Span) -> Result<Value, LanguageError> {
        let start = self.pos;
        if self.peek() == Some(b'-') {
            self.pos += 1;
        }
        ...
        let text = std::str::from_utf8(&self.bytes[start..self.pos])
            .map_err(|_| self.error("invalid number", span))?;
        if integral {
