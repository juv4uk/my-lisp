...
fn canonical_machine_gateway_rejects_raw_bytes_register_bypass_and_truncation_before_host() {
    ...
    for (request, expected) in [
        ...
        (
            // JZ rel32's rel32 slot fails closed the same way disp8 does:
            // one past the signed 32-bit max.
            "(x86-call-admitted-u64 (quote ((jz-rel32 2147483648))) 0)",
            "(rejected unadmitted-machine-form (jz-rel32 2147483648))",
        ),
        (
            // One past the signed 32-bit min.
            "(x86-call-admitted-u64 (quote ((jnz-rel32 -2147483649))) 0)",
            "(rejected unadmitted-machine-form (jnz-rel32 -2147483649))",
        ),
        (
            // JMP rel32's rel32 slot fails closed the same way.
