//! Contract for #115: host code may observe Lisp verdicts, but may not
...
fn authority_guard_rejects_host_authored_semantic_expectation_but_allows_mechanism_assertion() {
    ...
    assert!(!forbidden_run.status.success(), "host-authored semantic expectation must fail closed");
    let forbidden_stderr = String::from_utf8_lossy(&forbidden_run.stderr);
    assert!(forbidden_stderr.contains("semantic authority") && forbidden_stderr.contains("#113"),
    ...
    let allowed_run = Command::new("python3")
        ...
        .output()
        .expect("#115 authority guard must be executable");
    assert!(allowed_run.status.success(), "host mechanism assertion must remain legal: {}",
