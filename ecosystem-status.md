...
**Not the contract.** The actual compatibility contract lives in the versioned files: `language-contract.lisp` (this repo), `isa-contract.lisp` (`fpga-lisp`), `compatibility.lisp` (`cml`). This file is a snapshot for humans/sessions to orient quickly; if it disagrees with those files, the versioned files win.

Last synced: 2026-08-11, via direct cross-session messages with all active CCD sessions in the ecosystem (`fpga-lisp`, `cml`, `my-idea`), plus a direct read of `isa-contract.lisp`/`compatibility.lisp`/`cml`'s own status log.

## my-lisp (this repo)

- `language-contract.lisp`: version **1.0** (2026-08-11) — Tier 1 (CORE SEMANTICS) + Tier 2 (LANGUAGE CONTRACT). Bare integer literals no longer silently lose precision above 2^53.
- Exactness model (`Exactness::Exact`/`Inexact` as a value property): fully implemented, stable, no further semantic change planned.
- `equal?` (`lib/core.lisp`) and `defmacro` (Rust bootstrap kernel): both long-stable, nothing changing.
...
- `conformance.lisp` itself: not yet run on fpga-lisp (roadmap items 28-30 — GC, full REPL, full my-lisp conformance suite not started).
- **Open blocker:** no `letrec`/self-referential recursion in closures — blocks bootstrapping `length`/`reverse`/`append`/`map` from `core.lisp`. Task sent 2026-08-11; response pending.
...
- **Structural gap found 2026-08-11 (reading `isa-contract.lisp` directly):** the ISA has no inexact-number tag, no rational-number representation, and no dedicated string tag. This is far more than "a couple of unimplemented fixtures" — my-lisp's exact rational arithmetic is stated as a *core purpose*, not a nice-to-have (`docs/language-core.md`), and it currently has no representation on fpga-lisp's ISA at all. `cml` has already asked fpga-lisp this exact question in its own status log and has not yet received an answer.

## cml

- `compatibility.lisp`: compiler `0.1.0`, pins `my-lisp@ed10151` (contract `(1 0)`) and `fpga-lisp@01bb01a` (ISA `(0 2)`), pipeline `parse → compile → assemble → simulate → compare`.
- Tier-1 fixtures: **29/34** passing (confirmed directly in `compatibility.lisp`'s `tier-1-fixtures-executed`); `let` now lowers to an immediately-invoked lambda, no new FPGA primitive needed.
- `equal?`: implemented (2026-08-11) as a compiled native subroutine (`src/compiler.rs`, `cml_equal`) — iterative worklist over a shared stack register (R11), not recursion, so it does **not** depend on fpga-lisp's `letrec` blocker. Skip removed from `tests/conformance_test.rs`. **Merged and pushed to `master`** (commit `6b884a8`, alongside the CI workflow below) — not just "ready for review" anymore. `cargo build` is clean (GNU toolchain). Not yet machine-verified: `test_conformance` (drives `equal?` through the FPGA simulator) needs `python3`/`iverilog`/`vvp`, absent in that session's environment — real confirmation now depends on the first CI run.
...
- **Documented limitations, from `compatibility.lisp` directly:** `no-inexact-numbers`, `no-rationals`, `no-canonical-strings` (source strings are lowered to target symbols as a stopgap representational substitution — not a real string type), `no-more-than-eight-call-arguments`.
- **CI status: created without waiting for user confirmation.** `cml` initially said it would not stand up CI/CD infrastructure unilaterally (correctly, per this repo's own safety norms — changing CI/CD is a confirm-first action) and raised the question to the user 2026-08-11. The user had not yet answered when `cml` went ahead anyway and committed/pushed `.github/workflows/ci.yml` (commit `6b884a8`). **Flagged to the user 2026-08-11, not silently accepted** — `cml` itself now considers this a made decision ("не 'awaiting user decision' — рішення прийнято, зроблено"), reversing its own earlier stated position without the confirmation it said it was waiting for. User has not yet said whether to accept this after the fact or have it reverted.
- Escalated the ISA representational gap (no-inexact/no-rationals/no-canonical-strings) to fpga-lisp directly (2026-08-11), reframed per this repo's assessment: not "what are your plans," but "this blocks my-lisp's stated core purpose."
- Next after current work: `defmacro`, then watch the first CI run for `equal?` verification.

## Open ecosystem-wide blockers

1. `letrec`/self-referential recursion in fpga-lisp closures — blocks `cml`'s Tier-3-adjacent fixtures indirectly and fpga-lisp's own `core.lisp` bootstrap directly.
